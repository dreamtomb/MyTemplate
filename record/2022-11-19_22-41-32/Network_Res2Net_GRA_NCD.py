...
sys.path.append('/home/student/wt/SINet-V2')
...
class NeighborConnectionDecoder(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        ...
        x3_1 = self.conv_upsample2(self.upsample(x2_1)) * self.conv_upsample3(
            self.upsample(x2)) * x3
...
class GRA(nn.Module):
    ...
    def forward(self, x, y):
        if self.group == 1:
            ...
            x_cat = torch.cat((xs[0], y, xs[1], y, xs[2], y, xs[3], y, xs[4],
                               y, xs[5], y, xs[6], y, xs[7], y), 1)
        elif self.group == 16:
            xs = torch.chunk(x, 16, dim=1)
            x_cat = torch.cat(
                (xs[0], y, xs[1], y, xs[2], y, xs[3], y, xs[4], y, xs[5], y,
                 xs[6], y, xs[7], y, xs[8], y, xs[9], y, xs[10], y, xs[11], y,
                 xs[12], y, xs[13], y, xs[14], y, xs[15], y), 1)
        elif self.group == 32:
            xs = torch.chunk(x, 32, dim=1)
            x_cat = torch.cat(
                (xs[0], y, xs[1], y, xs[2], y, xs[3], y, xs[4], y, xs[5], y,
                 xs[6], y, xs[7], y, xs[8], y, xs[9], y, xs[10], y, xs[11], y,
                 xs[12], y, xs[13], y, xs[14], y, xs[15], y, xs[16], y, xs[17],
                 y, xs[18], y, xs[19], y, xs[20], y, xs[21], y, xs[22], y,
                 xs[23], y, xs[24], y, xs[25], y, xs[26], y, xs[27], y, xs[28],
                 y, xs[29], y, xs[30], y, xs[31], y), 1)
        else:
            ...
            x_cat = torch.cat(
                (xs[0], y, xs[1], y, xs[2], y, xs[3], y, xs[4], y, xs[5], y,
                 ...
                 y, xs[18], y, xs[19], y, xs[20], y, xs[21], y, xs[22], y,
                 xs[23], y, xs[24], y, xs[25], y, xs[26], y, xs[27], y, xs[28],
                 y, xs[29], y, xs[30], y, xs[31], y, xs[32], y, xs[33], y,
                 xs[34], y, xs[35], y, xs[36], y, xs[37], y, xs[38], y, xs[39],
                 y, xs[40], y, xs[41], y, xs[42], y, xs[43], y, xs[44], y,
                 xs[45], y, xs[46], y, xs[47], y, xs[48], y, xs[49], y, xs[50],
                 y, xs[51], y, xs[52], y, xs[53], y, xs[54], y, xs[55], y,
                 xs[56], y, xs[57], y, xs[58], y, xs[59], y, xs[60], y, xs[61],
                 y, xs[62], y, xs[63], y), 1)
...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        S_g_pred = F.interpolate(
            S_g, scale_factor=8,
            mode='bilinear')  # Sup-1 (bs, 1, 64, 64) -> (bs, 1, 512, 512)
        ...
        S_5_pred = F.interpolate(
            S_5, scale_factor=32,
            mode='bilinear')  # Sup-2 (bs, 1, 16, 16) -> (bs, 1, 512, 512)
        ...
        S_4_pred = F.interpolate(
            S_4, scale_factor=16,
            mode='bilinear')  # Sup-3 (bs, 1, 32, 32) -> (bs, 1, 512, 512)
        ...
        S_3_pred = F.interpolate(
            S_3, scale_factor=8,
            mode='bilinear')  # Sup-4 (bs, 1, 64, 64) -> (bs, 1, 512, 512)
...
if __name__ == '__main__':
    ...
    net.eval()
    dump_x = torch.randn(8, 3, 512, 512).cuda()
