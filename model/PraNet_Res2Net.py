...
from model.res2net50 import Res2Net
...
class RFB_modified(nn.Module):
    ...
    def initialize(self):
        weight_init(self)
        print("RFB_modified module init")
...
class aggregation(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        ...
        x3_1 = self.conv_upsample2(self.upsample(
            self.upsample(x1))) * self.conv_upsample3(self.upsample(x2)) * x3
    ...
    def initialize(self):
        weight_init(self)
        print("aggregation module init")


class PraNet(nn.Module):
    # res2net based encoder decoder
    def __init__(self, cfg, channel=32, model=None):
        super(PraNet, self).__init__()
        self.cfg = cfg
        self.model = model
        # ---- ResNet Backbone ----
        self.res2net = Res2Net(self.cfg)
    ...
    def forward(self, x):
        ...
        x = self.res2net.maxpool(x)  # bs, 64, 88, 88
        # ---- low-level features ----
        x1 = self.res2net.layer1(x)  # bs, 256, 88, 88
        x2 = self.res2net.layer2(x1)  # bs, 512, 44, 44

        x3 = self.res2net.layer3(x2)  # bs, 1024, 22, 22
        x4 = self.res2net.layer4(x3)  # bs, 2048, 11, 11
